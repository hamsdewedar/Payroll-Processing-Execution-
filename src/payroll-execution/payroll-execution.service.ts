...
export class PayrollExecutionService {
  ...
  async createPayrollRun(createPayrollRunDto: CreatePayrollRunDto): Promise<payrollRuns> {
    const payrollRun = new this.payrollRunModel(createPayrollRunDto);
    return await payrollRun.save();
  }

  async reviewPayroll(runId: string, reviewDto: PublishRunForApprovalDto): Promise<payrollRuns> {
  ...
  async generateEmployeePayrollDetails(employeePayrollDetailsDto: EmployeePayrollDetailsUpsertDto): Promise<employeePayrollDetails> {
    const employeePayrollDetails = new this.employeePayrollDetailsModel(employeePayrollDetailsDto);
  ...
  private async addExceptionToEmployee(
    ...
    try {
      ...
      console.error(`Error adding exception to employee ${employeeId}: ${error.message}`);
  ...
  // BR 9: Irregularity flagging with detailed tracking per employee
  async detectIrregularities(payrollRunId: string): Promise<string[]> {
    ...
    for (const detail of payrollDetails) {
      ...
      if (detail.netPay < 0) {
        ...
        await this.flagPayrollException(payrollRunId, 'NEGATIVE_NET_PAY', message, employeeId);
      }
      ...
      if (detail.bankStatus === 'missing') {
        ...
        await this.flagPayrollException(payrollRunId, 'MISSING_BANK_ACCOUNT', message, employeeId);
      }
      ...
      if (employee && detail.baseSalary > 0) {
          ...
          if (historicalData && historicalData.averageBaseSalary > 0) {
            ...
            if (isSpike) {
              ...
              await this.flagPayrollException(payrollRunId, 'SALARY_SPIKE', message, employeeId);
            }
        ...
        console.warn(`Error checking salary spike for employee ${employeeId}: ${error.message}`);
      }
    }
  ...
  // Requirement 0: Reviews/approvals before start of payroll initiation
  async getPreInitiationValidationStatus(): Promise<{
    canInitiate: boolean;
  ...
  async lockPayroll(runId: string): Promise<payrollRuns> {
    const payrollRun = await this.payrollRunModel.findById(runId);
    if (!payrollRun) throw new Error('Payroll run not found');
  ...
  async unlockPayroll(runId: string, unlockReason: string): Promise<payrollRuns> {
    const payrollRun = await this.payrollRunModel.findById(runId);
    if (!payrollRun) throw new Error('Payroll run not found');
  ...
  async freezePayroll(runId: string): Promise<payrollRuns> {
    // Freeze is functionally the same as lock - both prevent modifications
    return this.lockPayroll(runId);
  }
  ...
  async unfreezePayroll(runId: string, unfreezeReason: string): Promise<payrollRuns> {
    // Unfreeze is functionally the same as unlock - both allow modifications with reason
    return this.unlockPayroll(runId, unfreezeReason);
  }
  ...
  async processPayrollInitiation(payrollPeriod: Date, entity: string, payrollSpecialistId: string, currency?: string): Promise<payrollRuns> {
  ...
  async reviewPayrollInitiation(runId: string, approved: boolean, reviewerId: string, rejectionReason?: string): Promise<payrollRuns> {
    ...
    if (approved) {
      ...
      try {
        await this.generateDraftDetailsForPayrollRun(payrollRun._id.toString());
      } catch (error) {
        // If draft generation fails, update status and throw error
        console.error(`Error generating draft for payroll run ${runId}: ${error.message}`);
        throw new Error(`Failed to generate draft after approval: ${error.message}`);
      }
  ...
  async editPayrollInitiation(runId: string, updates: Partial<CreatePayrollRunDto>): Promise<payrollRuns> {
    const payrollRun = await this.payrollRunModel.findOne({ runId });
    if (!payrollRun) throw new Error('Payroll run not found');
  ...
  async processSigningBonuses(): Promise<employeeSigningBonus[]> {
  ...
  async reviewSigningBonus(reviewDto: SigningBonusReviewDto): Promise<employeeSigningBonus> {
  ...
  async editSigningBonus(editDto: SigningBonusEditDto): Promise<employeeSigningBonus> {
    ...
    if (editDto.signingBonusId) {
      ...
      try {
        ...
        console.warn(`Signing bonus config ${editDto.signingBonusId} not found: ${error.message}`);
      }
    }
  ...
  async processTerminationResignationBenefits(): Promise<EmployeeTerminationResignation[]> {
  ...
  async reviewTerminationBenefit(reviewDto: TerminationBenefitReviewDto): Promise<EmployeeTerminationResignation> {
  ...
  async editTerminationBenefit(editDto: TerminationBenefitEditDto): Promise<EmployeeTerminationResignation> {
    ...
    if (editDto.benefitId) {
      ...
      try {
        ...
        // If config not found, keep existing givenAmount (or use manual givenAmount if provided)
        console.warn(`Termination benefit config ${editDto.benefitId} not found: ${error.message}`);
      }
    }
  ...
  async calculatePayroll(employeeId: string, payrollRunId: string, baseSalary?: number): Promise<employeePayrollDetails> {
    ...
    if (needsProration && actualBaseSalary > 0) {
      try {
        ...
        console.error(`Error calculating prorated salary for employee ${employeeId}: ${error.message}`);
        await this.flagPayrollException(
          payrollRunId,
          'PRORATION_ERROR',
          `Failed to calculate prorated salary for employee ${employeeId}: ${error.message}`,
          employeeId
        );
      }
    }
  ...
  private async calculatePenaltiesWithBreakdown(
    ...
    if (employee.payGradeId) {
      try {
        ...
        console.warn(`Could not fetch PayGrade for employee ${employeeId}: ${error.message}`);
      }
    }
    ...
    try {
      ...
      console.warn(`Error calculating unpaid leave penalties for employee ${employeeId}: ${error.message}`);
    ...
    try {
      ...
      console.warn(`Error calculating time management penalties for employee ${employeeId}: ${error.message}`);
  ...
  private async calculateRefunds(employeeId: string, payrollRunId: string): Promise<number> {
    ...
    return 0;
      ...
      console.warn(`Error fetching refunds for employee ${employeeId}: ${error.message}`);
      return 0;
    }
  }
  ...
  async applyStatutoryRules(baseSalary: number, employeeId: string): Promise<number> {
  ...
  // BR 2: Contract terms validation
  // BR 20: Multi-currency support (currency stored in entity field)
  async generateDraftPayrollRun(payrollPeriod: Date, entity: string, payrollSpecialistId: string, currency?: string): Promise<payrollRuns> {
    // Validate inputs
    ...
    try {
      await this.generateDraftDetailsForPayrollRun(savedPayrollRun._id.toString());
    } catch (error) {
      ...
      throw new Error(`Failed to generate draft payroll details: ${error.message}`);
    }
  ...
  private async generateDraftDetailsForPayrollRun(payrollRunId: string): Promise<void> {
    ...
    await this.processTerminationResignationBenefits();
    ...
    for (const employee of activeEmployees) {
      try {
        ...
        await this.flagPayrollException(
          payrollRunId, 
          'CALC_ERROR', 
          `Error calculating payroll for employee ${employee._id}: ${error.message}`,
          employee._id.toString()
        );
      }
    }
  ...
  async getPayrollPreview(payrollRunId: string, targetCurrency?: string): Promise<any> {
    const payrollRun = await this.payrollRunModel.findById(payrollRunId);
    if (!payrollRun) throw new Error('Payroll run not found');
  ...
  async generateAndDistributePayslips(payrollRunId: string, distributionMethod: 'PDF' | 'EMAIL' | 'PORTAL' = 'PORTAL'): Promise<any> {
    const payrollRun = await this.payrollRunModel.findById(payrollRunId);
    if (!payrollRun) throw new Error('Payroll run not found');
    ...
    for (const detail of payrollDetails) {
      ...
      try {
        ...
        // Log error but don't fail the entire process
        console.error(`Error distributing payslip ${payslip._id} via ${distributionMethod}: ${error.message}`);
        // Flag as exception but continue with other payslips
        await this.flagPayrollException(
          payrollRunId,
          'PAYSLIP_DISTRIBUTION_ERROR',
          `Failed to distribute payslip for employee ${employeeId} via ${distributionMethod}: ${error.message}`,
          employeeId.toString()
        );
      }
    }
  ...
  private async distributePayslipAsPDF(payslip: any, employeeId: any): Promise<void> {
    try {
      ...
      console.error(`Error generating PDF for payslip ${payslip._id}: ${error.message}`);
      throw error;
    }
  }
  ...
  private async distributePayslipViaEmail(payslip: any, employeeId: any): Promise<void> {
    try {
      ...
      console.error(`Error sending email for payslip ${payslip._id}: ${error.message}`);
      throw error;
    }
  }
  ...
  private async distributePayslipViaPortal(payslip: any): Promise<void> {
    try {
      ...
      console.error(`Error distributing payslip ${payslip._id} via portal: ${error.message}`);
      throw error;
    }
  }
  ...
  async sendForApproval(payrollRunId: string, managerId: string, financeStaffId: string): Promise<payrollRuns> {
    const payrollRun = await this.payrollRunModel.findById(payrollRunId);
    if (!payrollRun) throw new Error('Payroll run not found');
  ...
  // BR: Enforce proper workflow sequence
  async approvePayrollDisbursement(financeDecisionDto: FinanceDecisionDto): Promise<payrollRuns> {
  ...
  async getEmployeeExceptions(employeeId: string, payrollRunId: string): Promise<{
  ...
  private async getEmployeeHistoricalPayrollData(
    ...
    try {
      ...
      console.error(`Error getting historical payroll data for employee ${employeeId}: ${error.message}`);
      return null;
    }
  }
  ...
  async getAllPayrollExceptions(payrollRunId: string): Promise<{
  ...
  // BR: Enforce proper workflow sequence
  async approvePayrollRun(managerApprovalDto: ManagerApprovalReviewDto): Promise<payrollRuns> {
